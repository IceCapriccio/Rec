...
`1.1 EDA&preprocess-train_data` and `1.2 EDA&preprocess-user_info` 是对train_data.txt和user_info.txt的EDA和预处理，输出user_item.pkl和user.pkl。（PS：pkl的读取速度是csv的好几倍，所以存储为pkl格式）[(下载链接)](https://pan.baidu.com/s/1F_7R6y8kpaMaubxtGltq5A?pwd=hsky)

`2. merge&transform` 将user和user-item连接，将showPos、refresh分桶，将network转为One-Hot向量，输出all_data.pkl。此notebook对内存要求较高，建议60G以上。[下载链接](https://pan.baidu.com/s/1s3861WeGSBI2ATcX0gaNPw?pwd=up3t)
...
处理过程中，尤其是`2. merge&transform`，简直是与内存斗智斗勇，感觉pandas的内存管理太差了（可能python都是这样吧），很多操作是拷贝和引用 根本不知道，甚至drop操作还会导致内存占用上升。
